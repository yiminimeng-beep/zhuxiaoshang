...
export interface StubResponse {
  status: number
  body?: unknown
  /** 永不 resolve：用来断言「请求还在飞的时候屏幕上是什么」（`CT-16`） */
  pending?: boolean
}
...
export function stubApi(map: StubMap = {}): StubApi {
  ...
  const fetchMock = vi.fn(async (input: unknown, init?: RequestInit) => {
    ...
    // 「请求在飞」是一种真实状态，得能停在它里面看屏幕——即时 resolve 的桩
    // 测不出来（`await user.click(...)` 会把微任务冲干净）
    if (step.pending) return new Promise<Response>(() => {})
    return makeResponse(step.body, step.status)
  })
